  ...
  <div class="bg-white dark:bg-gray-900 rounded-lg">
    ...
    >
      ...
      <template #image>
        <div class="border border-gray-300 dark:border-gray-700 rounded-b-md">
          <div
            class="flex flex-col gap-4 items-center justify-center w-full p-4"
          >
            <div v-if="image">
              <UChip
                size="md"
            ...
            <label
              ...
              @dragover.prevent
              @dragenter.prevent
              @drop="handleFileUploadOrDrop"
            >
              <div class="flex flex-col items-center justify-center pt-5 pb-6">
                <UIcon
                  class="h-8 w-8 mb-4 text-gray-500 dark:text-gray-400"
              ...
              <input
                ...
                type="file"
                class="hidden"
                @change="handleFileUploadOrDrop"
              />
            </label>
            <p v-if="props.currentImageUrl" class="text-sm">
              Actualmente:
              <ULink
                :to="getAbsoluteApiUrl(props.currentImageUrl)"
                class="underline"
    ...
    <div class="mt-2" v-if="props.showTips">
      <p class="text-gray-400 dark:text-gray-500 text-xs">
        <i>Nota: Puedes enviar texto e imagen al mismo tiempo.</i>
      </p>
      <p class="text-gray-400 dark:text-gray-500 text-xs">
        <i
          >Tip: Las ecuaciones deben estar entre "$$" para renderizarse: $$ f(x)
          = x $$</i
        >
      </p>
    </div>
...
const content = defineModel<string | undefined>("text", {
  default: "",
  required: false,
});
const fileImage = defineModel<File | undefined>("image", {
  default: undefined,
  required: false,
});
...
const handleFileUploadOrDrop = (event: Event | DragEvent) => {
  ...
  if (file.size > MAX_FILE_SIZE) {
    showNotification({
      message: "Tamaño de imagen excede el límite máximo de 5MB",
      type: "error",
    });
    return;
  }
