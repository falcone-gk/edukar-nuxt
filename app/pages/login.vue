<template>
  <UModal prevent-close v-model="isOpen">
    <UCard>
      ...
      <p>
        Por diversos motivos tuvimos que reniciar nuestros datos. Por ello,
        <span class="text-yellow-500 dark:text-yellow-400"
          >te invitamos a registrarte nuevamente</span
        >
        a nuestra comunidad de estudiantes.
      </p>

      <template #footer>
        <div class="flex justify-end gap-2">
          <CustomButtonGradient
            label="Ir a registrarse"
            color="gray"
            to="/signup"
          />
        </div>
    ...
    >
      ...
      <template #footer>
        <p>
          ...
          <ULink to="/account/reset-password" class="text-primary underline">
...
useSeoMeta({
  ...
  description:
    "Inicia sesión para acceder a los distintos recursos disponibles en nuestra página web.",
});
...
const validate = (state: any): FormError[] => {
  const errors = [];
  if (!state.username)
    errors.push({ path: "username", message: "Campo requerido" });
  if (!state.password)
    errors.push({ path: "password", message: "Campo requerido" });
  return errors;
};

const { data, status, error, execute } = useEdukarAPI<userInfo>(
  "account/login",
  {
    method: "post",
    body: state,
